...
### Автономний бандл (без `npm install` у споживача)

Для швидких експериментів, вендорингу або середовищ, де неможливо виконати
`npm install` для всього дерева залежностей, jkurwa вміє збиратись в один
самодостатній ESM-файл:

```sh
npm run build:bundle
```

Створює `dist/index.bundle.js` (~440 КБ), у який заінлайнено всі npm-залежності:
`bn.js`, форк `asn1.js` від muromec, `js-lzma`, `jksreader`, `@li0ard/kupyna` та
весь код `lib/`. Зовнішніми залишаються тільки вбудовані модулі Node
(`fs`, `node:crypto`, `buffer`).

Скопіюйте файл до свого проєкту та імпортуйте безпосередньо:
...
* Node ≥ 18.
* ESM-завантаження — або `"type": "module"` у `package.json` споживача, або
  розширення `.mjs` у файлі-імпортері. З CommonJS — динамічний імпорт:
  `const jk = await import("./vendor/jkurwa.bundle.js")`.
* Якщо використовуєте ГОСТ-хешування, симетричне шифрування або
  ГОСТ-захищені контейнери ключів — встановіть `gost89` на боці споживача:
  його свідомо не вбудовано в бандл, він і далі передається як об'єкт `algo`.

Бандл **не** створюється в `prepare`, тому не сповільнює звичайний
