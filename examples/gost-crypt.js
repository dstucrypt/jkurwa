...
const text = Buffer.from("{'msg': 'hello', 'code': 1}", 'binary');

// agreed keys
const sender_priv = jk.pkey('DSTU_PB_257', '40a0e1400001e091b160101150f1b1e0f1d14130e1c0b07011d120a04120c041d');
const reciever_pub = jk.pubkey('DSTU_PB_257', 'e54bf3f92a281d02f46ad5637387a8f13c9698816cb4f8beadfc0565fa63d6b1');

// random values choosen by sender
//  ukm (salt, 64 bytes), iv (cipher param, 8 bytes), cek - 32 bytes key

const ukm = Buffer.from('0a151914091304101b1a140f1c1d0b1d091c121f07091e0d1a0118011b02171a0f0b001c14011401121a0e1b090305021b190b081c02121a1f1d0a04080b1418', 'hex');
const iv = Buffer.from('09100509181c0515', 'hex');
const cek = Buffer.from('11080811020a0d0913040f020111190b04060c101d1c0a0911060e160b121419', 'hex');

// compute sharedkey and wrap encryption key
const sharedkey = sender_priv.sharedKey(reciever_pub, ukm, gost89.gosthash);
const wcek = gost89.wrap_key(cek, sharedkey, iv);

// finally encrypt text
const ciphered = gost89.compat.gost_encrypt_cfb(text, cek, iv);
