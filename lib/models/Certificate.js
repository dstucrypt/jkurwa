...
class Certificate {
  ...
  verify({ time }, hashes, lookupFn) {
    const issuer = lookupFn(this.issuerDN(), this.authorityKeyId);
    return (
      issuer &&
      (issuer.isRoot()
        ? issuer.trusted && issuer.verifySelfSigned({ time }, hashes)
        : issuer.verify({ time: this.valid.from }, hashes, lookupFn)) &&
      this.verifyTime(Number(time)) &&
      this.verifySignature(issuer.pubkey_unpack(), hashes) &&
      this.authorityKeyId.equals(
        issuer.pubkey.keyid({ hash: hashes.Dstu4145le })
      ) &&
      this.subjectKeyId.equals(this.pubkey.keyid({ hash: hashes.Dstu4145le }))
    );
  }

  verifySelfSigned({ time }, hashes) {
    return (
