...
var Message = function (asn1_ob) {
    var ob, parse, verify, decrypt, signer, originator, as_asn1, as_transport, mhash,
    ...
    parse = function (data) {
        ...
        transport_info = transport.decode(data);
        if (transport_info) {
            if (transport_info.docs[0].type === 'CERTCRYPT') {
                cert_b = transport_info.docs.shift().contents;
            }
            transport_info.doc_type = transport_info.docs[0].type;
            data = transport_info.docs.shift().contents;
            delete transport_info.docs;
        }
        ...
        if (cert_b) {
            try {
                this.help_cert = Certificate.from_asn1(cert_b);
            } catch (ignore) {
            }
        }
    };
    ...
    originator = function () {
        ...
        if (ri.value.originator.type === 'issuerAndSerialNumber') {
            return ri.value.originator.value;
        }
        if (ri.value.originator.type === 'originatorKey') {
            key = this.info.recipientInfos[0].value.originator;
            // ephemeral public key of sender
            return key.value.publicKey.data.slice(2);
        }
    };
    ...
    decrypt = function(crypter, algo) {
        ...
        var dest = rp.rid.value;
        
        // sselect privkey dynamycally depending on
        // message contents
        if (typeof crypter === 'function') {
            privkey = crypter(dest);
            if (!privkey && !Buffer.isBuffer(orig)) { 
                // we dont have key specified in destination
                // and origin is not ephemeral key, so it's possible
                // we have key for origin
                privkey = crypter(orig);
                orig = dest;
            }
            if (!privkey) {
                throw new Error("Don't have privkey for either origin or recipient");
            }
            crypter = privkey;
        }

        if (Buffer.isBuffer(orig)) {
            pubkey = crypter.curve.pubkey(util.add_zero(orig, true), 'buf8');
        } else {
            pubkey = algo.lookup(orig, 'crypt', [this.help_cert]);
            if (!pubkey) {
                throw new Error("Can't find certificate of sender");
            }
