...
var Message = function (asn1_ob) {
    var ob, parse, verify, verifyAttrs, decrypt, signer, as_asn1, as_transport, mhash,
    ...
    verifyAttrs = function (hash_f) {
        ...
        this.attrs.map(function(el) {
            attrs[el.type] = el.values[0];
        });
        if (!attrs.messageDigest) {
            return false;
        }
        if (attrs.messageDigest[0] !== 0x04 || attrs.messageDigest[1] !== attrs.messageDigest.length - 2) {
            return false;
        }
        var dgst = attrs.messageDigest.slice(2);
