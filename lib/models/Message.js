...
Message.prototype.addSignatureToken = function addSignatureToken(tspTokenB) {
  ...
  Object.assign(this.wrap.content.signerInfos[0], {
...
Message.prototype.timeAttr = function timeAttr(time) {
  const date =
    time === undefined ? new Date(Date.now()) : new Date(1000 * time);
...
Message.prototype.verifyAttrs = function verifyAttrs(
  hash_f,
  lookupCert,
  opts = {}
) {
...
Message.prototype.verifySigningTime = function verifySigningTime(
  time,
  lookupCert
) {
  if (!time) {
...
Message.prototype.verifyTimestampToken = function verifyTimestampToken(
  msg,
  hash_f,
  lookupCert,
  imprintOf
  ...
  return cmp(
    ...
    hash_f(
      { signature: this.signature(), content: hash_f(this.content()) }[
        imprintOf
      ]
    )
  );
...
Message.prototype.receiverKey = function receiverKey() {
