...
class SignedAttrs extends Attrs {
  ...
  set signingCertificateV2(value) {
    this.setAttr(
      ...
      certid.SigningCertificateV2.wrap(value.cert.ob, value.hash, value.hashName)
    );
  }
...
class Message {
  ...
  constructSigned(ob) {
    ...
    const wrap = {
      ...
      content: {
        version: 1,
        digestAlgorithms: [{ algorithm: ob.hash.algo || 'Gost34311' }],
        contentInfo: ob.data
        ...
        signerInfos: [
          {
            ...
            digestAlgorithm: { algorithm: ob.hash.algo || 'Gost34311' },
            digestEncryptionAlgorithm: {
              algorithm: {
                "Gost34311": "Dstu4145le",
                "Dstu7564-256": "Dstu4145leWithDstu7564"
              }[ob.hash.algo || 'Gost34311'],
            },
    ...
    this.pattrs.signingCertificateV2 = {
      cert: ob.cert,
      hash: ob.gostHash(ob.cert.as_asn1()),
      hashName: ob.gostHash.algo,
    };
    ...
    if (!signB) {
      this.addSignature({[ob.hash.algo]: ob.hash}, ob.signer);
    }
    this.addSignatureToken(tspTokenB);
  }

  addSignature(hashes, signer) {
    this.signature = signer.sign(this.mhash(hashes), "le");
    this.saveAttrs();
  }
  ...
  mhash(hashes) {
    ...
    const [{algorithm}] = this.info.digestAlgorithms;
    const hash_f = hashes[algorithm] || null;
    return hash_f ? hash_f(dataToSign) : null;
  }
  ...
  verifyAttrs(hashes, lookupCert, lookupCA, opts = {}) {
    if (!this.attrs) {
      return true;
    }

    let ok;
    ok = this.verifyAttrDigest(this.pattrs.messageDigest, hashes);
  ...
  verify(hashes, lookupCert, lookupCA, opts = {}) {
    const hash = this.mhash(hashes);
    if(!hash) {
      return false;
    }
    if (!this.verifyAttrs(hashes, lookupCert, lookupCA, opts)) {
      return false;
    }
