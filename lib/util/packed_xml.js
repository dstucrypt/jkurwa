...
var lzmaDecoder = require('./lzma.js'),
...
var Unpacker = function() {
    ...
    this.unpack = function(data) {
        ...
        xml = new xmldoc.XmlDocument(xmlBuf.toString());
        xmlBuf = undefined;

        if (xml.name !== "NAKL") {
            throw Error("Incorrect XML structure");
        }

        for (var i = 0; i < xml.children.length; i++) {
            el = xml.children[i];
            partName = el.name;
            for (var attrName in el.attr) {
                partName += "_" + attrName + "_" + el.attr[attrName];
            }
            res[partName] = b64_decode(el.val);
        }
