...
/**
 * Check whether a code matches a natural-person identifier format (10-digit
 * DRFO, 9-digit id-card number, or old AA123456 passport number).
 * @param {string} code Identifier code to test.
 * @returns {boolean} True if the code matches any natural-person format.
 */
...
/**
 * Test whether a certificate holder satisfies the requested role (see above).
 * @param {string} role Role name ("personal", "corporate", "fop", "director",
 *   "stamp", "other"), an exact DRFO code to match, or falsy to accept any.
 * @param {{cert?:{extension:{ipn:{DRFO?:string,EDRPOU?:string}}}}} ob Object
 *   holding the certificate whose IPN (DRFO/EDRPOU) codes are checked.
 * @returns {boolean} True if the certificate matches the role.
 */
