...
var finv = function(ob, inplace, _reuse_buf) {
    ...
    while (1) {
        ...
        while(1 && u[0] > 1) {
            ...
            mask = b0 & 1 ? 0: 0x7fffffff;
            b0 ^= p.bytes[0] & mask;

            console.log("ubits " + ubits );
            console.log("U " + bstring(u));
            console.log("B " + bstring(b));

            for(idx=0; idx<ob.mod_bits; idx++) {
                u1 = u.bytes[idx+1];
                u.bytes[idx] = ((u0>>1)|(u1<<31));
                u0 = u1;
                b1 = b.bytes[idx+1] ^ (p.bytes[idx+1] & mask);
                b.bytes[idx] = ((b0>>1)|(b1<<31));
                b0 = b1;
            }
        ...
        for(idx=0; idx<ob.mod_bits; idx++) {
            u.bytes[idx] ^= v.bytes[idx];
            b.bytes[idx] ^= c.bytes[idx];
        }
