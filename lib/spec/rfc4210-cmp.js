/*
 * Partial RFC 4210 (CMP) definitions.
 *
 * KNOWN LIMITATION: only the subset needed to fetch certificates from
 * Ukrainian CA CMP endpoints is modelled (see lib/services/cmp.js, which
 * reads just the certificate list out of the response). PKIBody supports
 * only ir/ip; several fields fall back to `.any()`/octstr placeholders and
 * are surfaced as raw DER rather than parsed structures. Building a
 * general-purpose CMP client would require completing these definitions.
 */
...
// OtherName / AnotherName: a typed name whose value shape is defined by the
// type-id OID; the value is kept as raw ANY DER here.
...
// IA5String wrapper, reused for the several GeneralName variants that are IA5.
var IA5String = asn1.define("IA5String", function () {
  this.ia5str();
});

var ORAddress = asn1.define("ORAddress", function () {
  // Limitation: the full X.400 ORAddress grammar (RFC 3280 A.1) is not
  // modelled; x400Address GeneralNames are kept as raw DER. Nothing in this
  // library reads them — Ukrainian PKI uses directoryName/URI names.
  this.any();
});

// EDIPartyName GeneralName variant (RFC 3280). The DirectoryString components
// are simplified to octet strings here rather than the full CHOICE.
...
// A key identifier octet string (e.g. senderKID/recipKID in the PKIHeader).
var KeyIdentifier = asn1.define("KeyIdentifier", function () {
  this.octstr();
});

var PKIFreeText = asn1.define("PKIFreeText", function () {
  // Limitation: RFC 4210 defines this as SEQUENCE OF UTF8String; it is read
  // as a raw octet string here, so callers get undecoded bytes. Only used
  // for the informational statusString of PKIStatusInfo.
  this.octstr();
});

// InfoTypeAndValue: an OID-typed generic info item carried in PKIHeader's
// generalInfo (and general messages); infoValue kept as raw ANY DER.
...
var PKIHeader = asn1.define("PKIHeader", function () {
  this.seq().obj(
    ...
    this.key("protectionAlg")
      .optional()
      .implicit(1)
      .use(rfc3280.AlgorithmIdentifier),
...
// CRMF OptionalValidity (RFC 4211): a validity window where either bound may be
// omitted in a certificate request template.
...
// CRMF CertTemplate: the selected certificate fields a requester proposes; all
// fields are optional. Quirk: per RFC 4211 the extensions field is context tag
// [9], but this schema tags it implicit(8) — the same tag used for subjectUID
// (see the ASN.1 reference block above). Preserved as-is for compatibility.
var CertTemplate = asn1.define("CertTemplate", function () {
  this.seq().obj(
    this.key("version").optional().implicit(0).use(rfc3280.Version),
    this.key("serialNumber")
      .optional()
      .implicit(1)
      .use(rfc3280.CertificateSerialNumber),
    this.key("signingAlg")
      .optional()
      .implicit(2)
      .use(rfc3280.AlgorithmIdentifier),
    ...
    this.key("publicKey")
      .optional()
      .implicit(6)
      .use(rfc3280.SubjectPublicKeyInfo),
...
// CRMF ProofOfPossession. Only the raVerified [0] variant (RA already verified
// possession of the private key) is modelled; the signature/keyEnc variants are
// not needed by this library.
...
// CRMF CertRequest: a request id, the CertTemplate, and optional controls.
var CertRequest = asn1.define("CertRequest", function () {
  this.seq().obj(
    this.key("certReqId").int(),
    this.key("certTemplate").use(CertTemplate),
    // Limitation: Controls (SEQUENCE OF AttributeTypeAndValue, RFC 4211
    // s6) is not modelled; kept as raw DER. Requests built by this library
    // never set it.
    this.key("controls").optional().any()
  );
});

// CRMF CertReqMsg: a CertRequest paired with its optional proof-of-possession.
...
// SEQUENCE OF CertReqMsg — the body of an ir/cr/kur request.
var CertReqMessages = asn1.define("CertReqMessages", function () {
  this.seqof(CertReqMsg);
});

// PKIStatus code (0 = accepted, 2 = rejection, etc. per RFC 4210 s5.2.3).
var PKIStatus = asn1.define("PKIStatus", function () {
  this.int();
});

// PKIStatusInfo: the outcome status plus optional human-readable text and
// failure info returned in a response.
var PKIStatusInfo = asn1.define("PKIStatusInfo", function () {
  this.seq().obj(
    ...
    // Limitation: PKIFailureInfo (BIT STRING of failure flags, RFC 4210
    // s5.2.3) is not decoded; consumers see raw DER. Failure responses are
    // currently detected by the numeric status alone.
    this.key("failInfo").optional().any()
  );
});

// CertResponse: the per-request status. Note: this profile stops at the status
// and does not parse the certifiedKeyPair (the issued certificate) that RFC
// 4210 places here — see cmp.js, which reads certificates from caPubs instead.
...
// CertRepMessage (the ip/cp response body): the CA/chain certificates in
// caPubs [0] plus the per-request responses. (The asn1.js name label
// "CertReqMessage" here is a pre-existing misnomer; kept unchanged.)
...
*/
// PKIBody CHOICE — the message payload. Of the 27 CMP message types listed
// above, only ir [0] (initialization request) and ip [1] (initialization
// response) are implemented, which is all the certificate-fetch flow needs.
...
// CMPCertificate: an X.509 certificate as carried in a PKIMessage (extraCerts).
var CMPCertificate = asn1.define("CMPCertificate", function () {
  this.use(rfc3280.Certificate);
});

// PKIProtection: the bit string protecting the message (MAC or signature over
// the protected part of header+body); not verified by this library.
