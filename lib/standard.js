/*
 * Standard DSTU 4145 polynomial-basis (PB) elliptic curves.
 *
 * Each definition holds the curve parameters as hex strings (spaces are
 * cosmetic and ignored by the parser): field polynomial exponents `ks`/`m`,
 * curve coefficients `a`/`b`, the `base` generator point (x, y), the subgroup
 * `order`, and the cofactor `kofactor`. `cache` is a shared, initially empty
 * lookup used elsewhere to memoize instantiated Curve objects.
 */
const DSTU_PB_257 = {
  a: "0",
...
export { DSTU_PB_257, DSTU_PB_191, DSTU_PB_431, cache };
