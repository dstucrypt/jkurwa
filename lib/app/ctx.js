...
Box.prototype.sign = async function sign(data, role, cert, opts) {
...
Box.prototype.unwrap = function unwrap(data, content, opts={}) {
  let msg;
  let x;
  const info = { pipe: [] };
  let tr;
  ...
  while (data && data.length) {
    ...
    if (msg.type === "signedData") {
      ...
      info.pipe.push({
        ...
        contentTime: opts.tsp && msg.contentTime() || null,
        tokenTime: opts.tsp === 'all' && msg.tokenTime() || null,
      });
    }
