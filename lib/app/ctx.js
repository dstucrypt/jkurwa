...
class Box {
  ...
  verifyCert(cert, time, usage) {
    ...
    let cachedRes = (sidHex && this.verifiedCache.hasOwnProperty(sidHex))
    ...
    if (cachedRes && cachedRes.ret === true) {
      return (
          (usage ? cert.canUseFor(usage) : true) &&
          cert.verifyTime(Number(time))
      );
    }
  ...
  getHashAlgo(name) {
    const hashFn = data=> this.algo.hashes[name || 'Gost34311'](data);
    hashFn.algo = name;
    return hashFn;
  }
  ...
  async sign(data, role, cert, opts) {
    ...
    const dataHash = this.getHashAlgo(opts.hash)(data);
    let tspB;
    ...
    const message = new Message({
      ...
      dataHash,
      signer: key.priv,
      hash: this.getHashAlgo(opts.hash),
      gostHash: this.getHashAlgo('Gost34311'),
      tspB,
      signTime: opts.time
    });
