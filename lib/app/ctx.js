...
class Box {
  ...
  async lookupOCSP(lookup, query, msg) {
    ...
    try {
      return response.verify(ocspCtx, cert, query.serialNumber, nonce);
    } catch (e) {
  ...
  async sign(data, role, unusedCert, opts) {
    ...
    if (opts.ocsp) {
      let nonce = rand(Buffer.alloc(20));
      let ocspResponse = await ocspService.lookup(
        key.cert,
        key.cert.serial,
        nonce,
        this.ocspCtx
      );
      message.addOcspHashes([[ocspResponse.makeRef(this.ocspCtx)]]);
      message.addOcspResponses([ocspResponse]);
    }
    return message;
  }
